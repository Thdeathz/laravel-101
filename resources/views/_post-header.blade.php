<header class="max-w-xl mx-auto mt-20 text-center">
  ...
  <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-8">
    <!--  Category -->
    <x-dropdown>
      <x-slot name="trigger">{{ isset($currentCategory) ? $currentCategory->name : 'Categories' }}</x-slot>

      <a href="/"
        class="block mt-1 transition-colors duration-200 hover:bg-blue-300 hover:text-white px-3 py-1 focus:bg-blue-300">All</a>

      @foreach ($categories as $category)
      <a href="/categories/{{ $category->slug }}" class="
            block mt-1 transition-colors duration-200 hover:bg-blue-300 hover:text-white px-3 py-1 focus:bg-blue-300
